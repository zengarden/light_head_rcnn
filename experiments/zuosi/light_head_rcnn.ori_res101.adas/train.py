...
def train(args):
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        if args.checkpoint is not None:
            match = re.findall(r'\d+', args.checkpoint)
            if len(match) != 2:
                raise Exception("invalid checkpoint: {}".format(args.checkpoint))
            print('restore from checkpoint: {}'.format(args.checkpoint))
            restorer.restore(sess, args.checkpoint)
            start_epoch, start_step = [int(e) for e in match]
        ...
        for epoch in range(start_epoch, cfg.max_epoch):
            if not restore_from_checkpoint and cfg.warm_iter > 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-c', '--checkpoint', default=None, type=str, help='checkpoint path')
    parser.add_argument(
