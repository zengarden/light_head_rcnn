 ...
 public:
  ...
  void Compute(OpKernelContext* context) override 
  {
    ...
    PSROIPoolingKernel(context, &bottom_data, spatial_scale_, num_rois, num_channels, data_height, data_width, 
                       pooled_height, pooled_width, &bottom_rois, out_channels, group_size_, output_shape);
...
bool PSROIPoolBackwardLauncher(
    const float* top_diff, const int* mapping_channel, const int batch_size,
...
static void PSROIPoolingGradKernel(
    ...
    const Tensor* out_backprop, const Tensor* mapping_channel,
    ...
    const int output_dim, const Tensor* bottom_rois, const TensorShape& tensor_output_shape) 
{
  ...
  PSROIPoolBackwardLauncher(
    out_backprop->flat<float>().data(), mapping_channel->flat<int>().data(),
    batch_size, num_rois, spatial_scale, channels, height, width,
    pooled_height, pooled_width, output_dim, output->flat<float>().data(),
    bottom_rois->flat<float>().data(), context->eigen_device<Eigen::GpuDevice>());
}
 ...
 public:
  ...
  void Compute(OpKernelContext* context) override 
  {
    ...
    PSROIPoolingGradKernel(
        context, &bottom_data, &out_backprop, &mapping_channel, batch_size,
        num_rois, spatial_scale_, channels, data_height, data_width,
        pooled_height, pooled_width, output_dim, &bottom_rois, output_shape
    );
