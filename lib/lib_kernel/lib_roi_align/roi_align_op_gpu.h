...
#ifndef TENSORFLOW_USER_OPS_ROIALIGN_OP_GPU_H_
#define TENSORFLOW_USER_OPS_ROIALIGN_OP_GPU_H_
...
bool RoiAlignForwardLaucher(
    const float* bottom_data, const float spatial_scale, 
    const int num_rois, const int height, const int width, const int channels, 
    const int pooled_height, const int pooled_width, 
    const int sample_height, const int sample_width, const float* bottom_rois,
    float* top_data, int* argmax_data, const Eigen::GpuDevice& d);

bool RoiAlignBackwardLaucher(
    const float* top_diff, const float spatial_scale, 
