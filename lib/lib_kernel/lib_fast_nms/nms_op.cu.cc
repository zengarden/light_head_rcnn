...
__device__ __forceinline__ bool box_iou(Box a, Box b, float thresh) {
    ...
    float width = max(right - left + 1.f, 0.f),
          height = max(bottom - top + 1.f, 0.f);
    float interS = width * height;
    float Sa = (a.x1 - a.x0 + 1.f) * (a.y1 - a.y0 + 1.f);
    float Sb = (b.x1 - b.x0 + 1.f) * (b.y1 - b.y0 + 1.f);
    return interS > (Sa + Sb - interS) * thresh;
}

//! store uint64_type with cache streaming 
__device__ __forceinline__ void store_u64_cs(uint64_type *ptr, uint64_type val) {
...
__global__ void kern_gen_mask(
        ...
        const Box *dev_boxes, const int dev_mask_width, uint64_type *dev_mask) {
    const int
        box_group_row = blockIdx.y,
        box_group_col = blockIdx.x;

    //if (box_group_row == 0 && box_group_col == 0) {
    //    printf("%f, %f , %f, %f\n",  dev_boxes[1].x0, dev_boxes[1].y0, dev_boxes[1].x1, dev_boxes[1].y1); 
    //}
...
#pragma unroll
    for (uint32_type offset = WARP_SIZE / 2; offset; offset /= 2)
...
__device__ __forceinline__ void bitwise_or_single_warp(BitwiseOrArgs args) {
    ...
    for (uint32_type i = threadIdx.x; i < size; i += WARP_SIZE) {
        dst[i] |= load_u64_cs(&src[i]);
    }
}

__global__ void kern_gen_indices(
        uint32_type nr_boxes, uint32_type max_output, uint32_type overlap_mask_width,
        const uint64_type * __restrict__ overlap_mask, uint64_type *__restrict__ rm_mask,
        int * __restrict__ out_idx, int * __restrict__ out_size) {
    __shared__ uint32_type out_pos;
    __shared__ BitwiseOrArgs bitwise_or_args;

    const uint32_type nr_box_blocks = DIVUP(nr_boxes, 64);
    ...
    while (th0_box_block_id < nr_box_blocks) {
        ...
        if (min_box_block_id + 1) {
            ...
            if (min_box_block_id == box_block_id) {
                ...
                out_idx[out_pos ++] = static_cast<int>(box_id);
            }
            __syncthreads();
    ...
    if (out_pos < max_output) {
        ...
        int val = out_idx[out_pos - 1];
        for (uint32_type i = out_pos + threadIdx.x; i < max_output; i += WARP_SIZE) {
            out_idx[i] = val;
        }
    }
    if (!threadIdx.x) {
        *out_size = static_cast<int>(out_pos);
    }
}
...
void launch_gen_mask(
    ...
    int overlap_mask_bytes = nr_boxes * DIVUP(nr_boxes, 64) * sizeof(unsigned long long);
    cudaMemsetAsync(
        dev_mask, 0, overlap_mask_bytes, d.stream());


    kern_gen_mask<<<blocks, threads, 0, d.stream()>>>(
...
void launch_gen_indices(
        ...
        const unsigned long long *overlap_mask, unsigned long long *rm_mask,
        int *out_idx, int *out_size,
        const Eigen::GpuDevice& d) {
    cudaMemsetAsync(
        rm_mask, 0, sizeof(unsigned long long) * DIVUP(nr_boxes, 64), d.stream());
    kern_gen_indices<<<1, WARP_SIZE, 0, d.stream()>>>(
