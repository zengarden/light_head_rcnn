...
#     dtype=tf.float32)
rois_npy = np.load('rois.npy')
rois_npy = rois_npy[:, :4]
rois = tf.convert_to_tensor(rois_npy, dtype=tf.float32)

nms_out = nms_op.nms(rois, 0.5, 200)
#nms_out_1 = nms_op.nms(rois, 0.5, 200)
...
# print(sess.run(rois))
# print("-------")
print(sess.run(nms_out)[0])
#print(sess.run(nms_out_1))
#print(sess.run(hehe))
sess.close()
